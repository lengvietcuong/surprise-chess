...
public class Chess960 : IBoardSetup
{
    ...
    public void SetUp(Board board)
    {
        ...
        {
            // Place pawns
            board.SetPieceAt(new Position(1, col), PieceFactory.Create(PieceColor.White, PieceType.Pawn));
            board.SetPieceAt(new Position(6, col), PieceFactory.Create(PieceColor.Black, PieceType.Pawn));

            // Place back rank pieces
            PieceType type = backRank[col];
            board.SetPieceAt(new Position(0, col), PieceFactory.Create(PieceColor.White, type));
            board.SetPieceAt(new Position(7, col), PieceFactory.Create(PieceColor.Black, type));
        }
    }
